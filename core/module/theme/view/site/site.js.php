...
$("input, select").on("change", function() {
	...
	css += ".speechBubble,.button:not(.buttonGrey):not(.buttonRed),button[type='submit'],.pagination a,input[type='checkbox']:checked + label:before,.helpContent{background-color:" + colors.normal + ";color:" + colors.text + "!important}";
	...
	css += ".button:not(.buttonGrey):not(.buttonRed):hover,button[type='submit']:hover,.pagination a:hover,input[type='checkbox']:not(:active):checked:hover + label:before,input[type='checkbox']:active + label:before{background-color:" + colors.darken + "}";
	...
	// Couleur du texte
	css += "body,.block h4,input[type='email'],input[type='text'],input[type='password'],.inputFile,select,textarea,.inputFile,.button.buttonGrey{color:" + $("#themeTextTextColor").val() + "}";
	// Largeur du site
