...
$("input, select").on("change", function() {
	...
	css += "h1,h2,h3,h4,h5,h6{font-family:'" + titleFont.replace("+", " ") + "',sans-serif}";
	// Largeur du site
	css += ".container{max-width:" + $("#themeSiteWidth").val() + "}";
	// Arrondi sur les coins du site
	css += "#site{border-radius:" + $("#themeSiteRadius").val() + "}";
	// Ombre sur les bords du site
	css += "#site{box-shadow:" + $("#themeSiteShadow").val() + " #3C3C3C}";
	// Ajout du css au DOM
	$("#themePreview").remove();
	$("<style>")
		.attr("type", "text/css")
